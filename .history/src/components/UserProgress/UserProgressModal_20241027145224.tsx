...
import { topicSelectorAll } from '../../lib/resource-progress.ts';
...
export function UserProgressModal(props: ProgressMapProps) {
  ...
  async function getUserProgress(
    ...
    const { error, response } = await httpGet<UserProgressResponse>(
      ...
      }/resource/get-user-progress/${userId}?resourceType=${resourceType}&resourceId=${resourceId}`,
    );
  ...
  useEffect(() => {
    ...
    Promise.all([
      getRoadmapSVG(resourceJsonUrl, renderer),   // 获取渲染svg
      getUserProgress(userId, resourceType, resourceId),   // 获取用户进度数据
    ])
      .then(([svg, user]) => {
        if (!user || !svg) {
          return;
...
// 遍历每一个donelist里的topic DOM节点给他们添加样式
